...
%
% Model name = Wajima2009_BloodCoagulation_aPTTtest
%
% is http://identifiers.org/biomodels.db/MODEL1107010000
% is http://identifiers.org/biomodels.db/BIOMD0000000338
...
function state = Patient_1(heparin_infusion,plotflg) 
%based on BIOMD0000000338
global g_heparin_infusion hemophiliaA hemophiliaB
heparin_const = 1e4*2.22;
if nargin<1, g_heparin_infusion= 5*heparin_const;
else g_heparin_infusion = heparin_infusion*heparin_const;
end
if nargin<2, plotflg=0; end
hemophiliaA=0; hemophiliaB=0;
...
x0(15) = 26.7;   % V
x0(16) = 0.0;    % Va
x0(17) = 1394.4; % II       Prothrombin
x0(18) = 0.0;    % F       Fibrin
x0(19) = 8945.5; %  Fg      Fibrinogen
x0(20) = 0.0;    % DP
x0(21) = 0.0;    % P   Plasminogen
x0(22) = 0.0;    % XF
x0(23) = 70.3;   % XIII
x0(24) = 2154.3; % Pg Plasmin
x0(25) = 0.0;    % APC
x0(26) = 0.0;    % IIa_Tmod
x0(27) = 60.0;   % PC Vitamin K-dependent protein C
x0(28) = 50.0;   % Tmod  Thrombomodulin
x0(29) = 0.0;    % TF    Tissue factor
x0(30) = 0.0;    % VIIa_TF
x0(31) = 0.0;    % VII_TF
x0(32) = 0.0;    % Xa TFPI
x0(33) = 2.5;    % TFPI    Tissue factor pathway inhibitor
x0(34) = 300.0;  % PS   Vitamin K-dependent protein S
x0(35) = 0.1;    % VKH2
x0(36) = 0.0;    % Va_Xa
x0(37) = 0.0;    % CA
x0(38) = 375.0;  % XII
x0(39) = 0.0;    % K
x0(40) = 0.0;    % initial Heparin-level ATIII_Heparin
x0(41) = 0.0;    % Xa_ATIII     Heparin
x0(42) = 1.0;    % VK       vitamin K    
x0(43) = 0.0;    % C_warf   warfarin
x0(44) = 0.1;    % VKO      vitamin K
x0(45) = 450.0;  % Pk      Plasma kallikrein
x0(46) = 0.0;    % FDP    
x0(47) = 0.0;    % D 
x0(48) = 0.0;    % TAT    
x0(49) = 0.0;    % VIIa_TF_Xa_TFPI
x0(50) = 0.0;    % XIIIa
x0(51) = 0.0;    % IIa_ATIII_Heparin
x0(52) = 0.0;    % IXa_ATIII_Heparin
x0(53) = 0.0;    % A_warf    warfarin
x0(54) = 0.0;  % VK_p    	vitamin K
%%

if hemophiliaA
    x0(2) = x0(2)*25/100;     % VIII  
elseif hemophiliaB
    x0(5) = x0(5)*25/100;    % IX
end
%%
colors={'b','g','r','c','m','y'};
indx_Factor_Xa=13;indx_Factor_X=12;indx_Thrombin=1;indx_Prothrombin=17;indx_Fibrin=18;indx_Fibrinogen=19;
dt=0.001; tspan=[0:dt:150/60/60];
% run ODE
opts = odeset('AbsTol',1e-3);
[t,x]=ode45(@f,tspan,x0,opts);
% calculate aPTT
integral_Fibrin = cumsum(x(1:end,indx_Fibrin));
ind=find(integral_Fibrin>=1500,1,'first'); aPTT = t(ind)*60*60; if isempty(aPTT), aPTT=inf;end
state = [aPTT g_heparin_infusion/heparin_const];
%
if plotflg
subplot(1,2,1); kk=0;
for k=[indx_Factor_Xa indx_Factor_X indx_Thrombin indx_Prothrombin indx_Fibrin indx_Fibrinogen]
    kk=kk+1;plot(t(1:end)*60*60,x(1:end,k)./max((x(1:end,k)))*100,colors{kk},'LineWidth',1.5);hold on
end
legend('Xa','X','Thrombin','Prothrombin','Fibrin','Fibrinogen'), xlabel('Time (s)'),ylabel('% initial value'), axis square
subplot(1,2,2); 
semilogy(t(1:end)*60*60,integral_Fibrin,'-ob','LineWidth',1.5); axis square
hold on,plot(aPTT,integral_Fibrin(ind),'r+','MarkerSize',10)
text(aPTT+1,integral_Fibrin(ind)-1000,[ num2str(round(aPTT*10)/10) ' sec'],'fontsize',12)
xlabel('Time (s)'),ylabel('Integral of Fibrin'),title('aPTT Test')
end
...
global g_heparin_infusion
heparin_infusion = g_heparin_infusion;

compartment_compartment_1=1;
global_event_dilution_time=0;
...
% Parameter:   id =  Integral_Fibrin, name = Integral_Fibrin
...
x(52) = x(56);
% rateRule: variable = Integral_Fibrin
x(57) = x(57)+x(18);
...
reaction_eHeparinIXa=compartment_compartment_1*global_par_Heparin_ke*x(53);
...
%Event: id=dilution_event
event_dilution_event=time > global_event_dilution_time/60/60;

if(event_dilution_event)
    compartment_compartment_1=compartment_compartment_1*3;
end
reaction_pHeparin = compartment_compartment_1*Constant_flux_irreversible(heparin_infusion);


xdot=zeros(57,1);
% rateRule: variable = C_warf
xdot(55) = global_par_Warfarin_ka*x(52)/global_par_Warfarin_Vd-global_par_Warfarin_ke*x(43);
% rateRule: variable = A_warf
xdot(56) = (-global_par_Warfarin_ka)*x(52);
% rateRule: variable = Integral_Fibrin
xdot(57) = x(18);
...
% Species:   id = A_warf, name = A_warf, involved in a rule 	xdot(52) = x(52);

% Species:   id = IXa_ATIII_Heparin, name = IXa_ATIII_Heparin, affected by kineticLaw
xdot(53) = (1/(compartment_compartment_1))*(( 1.0 * reaction_r46) + (-1.0 * reaction_eHeparinIXa));
